...
export interface Props {
   ...
   *   "draft" — «Чертёж»: обычная отрисовка на холсте. Он векторный по сути,
   *             печатается и выгружается в SVG — это основной рабочий режим.
   *   "model" — «Модель»: объёмный просмотр на three.js/WebGL. Нужен, чтобы
   *             облететь схему и показать её в объёме; редактирование там не
   *             ведётся, печать не снимается.
   */
  viewMode?: "draft" | "model";
