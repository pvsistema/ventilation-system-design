...
export interface TopoBranch {
  ...
  dPTotal?: number;
  /**
   * кМюрг — ОБЩЕЕ сопротивление ветви: выработка + перемычка/окно + окно ГВУ.
   * Ровно та сумма, которая уходит в решатель сети.
   *
   * Поле resistance содержит сопротивление ТОЛЬКО выработки: перемычка обычно
   * задана значком на схеме, и её R живёт в SchemaSymbol.bk*. Поэтому расчётам
   * пожара (Прил. 5 и Прил. 7) нужна именно эта величина — иначе ветвь с
   * закрытой дверью считается как пустая выработка.
   *
   * Проставляется в Cad.tsx (branchesWithTotalDep) перед передачей в аварийные
   * расчёты; в самой схеме поле отсутствует — потребители обязаны иметь
   * запасной вариант на resistance.
   */
  rTotal?: number;
  ...
  // Температуры воздуха на концах ветви при пожаре, °C (в ориентации from→to).
  // По ним решатель считает вес горячего столба. Задаются моделью дыма и
  // относятся к СТРУЕ, а не к узлу: свежая выработка, впадающая в задымлённый
  // узел, остаётся холодной и не создаёт фиктивной тяги.
  fireTFrom?: number;
  fireTTo?: number;
  ...
  // Выработка, вдоль которой проложена нить става. Нить рисуется узкой (20% от
  // ширины выработки), поэтому УО на ней — вентилятор, замерная станция —
  // масштабируются от ширины ХОЗЯЙСКОЙ выработки, иначе значок выходит
  // крошечным на экране и несоразмерно крупным при печати.
  vpHostBranchId?: string;
  ...
  /**
   * Считать длину загазованного участка по метановыделению (РБ №343, ф. 1/3/5),
   * а не брать введённую руками.
   *
   * Длина зоны — самый влиятельный вход расчёта взрыва: от неё зависят и объём
   * смеси, и начальное давление очага. Методика даёт её расчётом, но исходные
   * данные (дебит метана, мощность пласта) есть не всегда, поэтому ручной ввод
   * остаётся и включён по умолчанию — старые проекты считаются как прежде.
   */
  explosionGasZoneAuto?: boolean;
  /** Где формируется зона: очистной забой / непогашенная / подготовительная. */
  explosionGasZoneKind?: "longwall" | "preserved" | "heading";
  /** Фактическое метановыделение I, м³/мин (ф. 1, 3, 5). */
  explosionGasEmission?: number;
  /** Вынимаемая мощность пласта m, м — для сечения закрепного пространства (ф. 2). */
  explosionSeamThickness?: number;
  /** Шаг обрушения непосредственной кровли l_н.к, м (ф. 2). */
  explosionCaveStep?: number;
