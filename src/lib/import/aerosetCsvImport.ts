// ─────────────────────────────────────────────────────────────────────────────
// aerosetCsvImport.ts — CSV-ВЫГРУЗКА ИЗ ПО «АЭРОСЕТЬ».
//
// Формат с ТЕКСТОВЫМИ ЗАГОЛОВКАМИ столбцов: номера колонок не задаются
// вручную, а определяются по названиям в шапке таблицы.
//
// АэроСеть экспортирует 5 файлов (либо один файл со всеми секциями):
//   *-nodes.csv        — узлы: ID; X; Y; Z; Атмосфера
//   *-excavations.csv  — выработки: ID; НачВерш; КонВерш; Название; Длина;
//                        Тип; S; P; Q; R; Слой; ИдПозиции
//   *-positions.csv    — позиции ПЛА
//   *-bulkheads.csv    — перемычки
//   *-fans.csv         — вентиляторы
//
// НЕ ПУТАТЬ: выгрузку Вентиляции 2.0 разбирает vent2CsvImport.ts (там номера
// столбцов задаёт пользователь), а CSV из Ventsim — ventsimImport.ts.
...
import {
  ...
  parseNum, detectSep, splitRow, normalizeLines, buildHorizonsFromLayers,
  resistanceUnitFromHeader, detectResistanceIncludesBulkheads, detectResistanceUnit,
} from "@/lib/import/importCommon";
import {
  isDataId, cleanId, parseNumSci, parseCsvColor, fanNameFromType, fanTypeFromRaw,
} from "@/lib/import/csvFieldUtils";
...
function buildResult(
  ...
  if (rIncludesBk) {
    ...
    warnings.push(
      ...
      "Перемычки нанесены на схему со своим сопротивлением, а из выработки оно " +
      "вычтено, чтобы не посчиталось дважды."
    );
  }
