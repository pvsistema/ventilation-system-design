...
/**
 * Пропорции типового арочного сечения — 4 м ширины, 3 м прямой стенки,
 * стрела свода 1 м. По ним строится выработка, у которой в файле есть только
 * площадь (форма «custom»): габаритов там нет вовсе, а рисовать что-то надо.
 * Абсолютные числа неважны — контур всё равно приводится к фактической S.
 */
const CUSTOM_W = 4, CUSTOM_H = 3, CUSTOM_ARCH = 1;

/** Площадь замкнутого многоугольника контура, м² (формула шнурков). */
function polygonArea(pts: SectionPoint[]): number {
  let s = 0;
...
/**
 * Контур сечения по ГАБАРИТАМ ветви, без приведения к площади.
 *
 * Отдельно от sectionOutline, потому что даёт только ФОРМУ: пропорции стенок,
 * свода, скоса трапеции. Истинный размер накладывается следующим шагом.
 */
function rawOutline(b: TopoBranch): SectionPoint[] {
  ...
  // Форма «custom» — в файле только S и P, габаритов нет. Берём пропорции
  // типовой арки: истинный размер всё равно придёт из площади (см. ниже).
  const custom = shape === "custom" || !((b.rectWidth ?? 0) > 0.01) || !((b.rectHeight ?? 0) > 0.01);
  const w = custom ? CUSTOM_W : Math.max(0.1, b.rectWidth);
  const h = custom ? CUSTOM_H : Math.max(0.1, b.rectHeight);
  const halfW = w / 2;

  if (custom) {
    // Арочный контур по типовым пропорциям.
    const pts: SectionPoint[] = [
      { r: -halfW, u: -h / 2 },
      { r:  halfW, u: -h / 2 },
      { r:  halfW, u:  h / 2 },
    ...
    for (let i = 1; i < steps; i++) {
      ...
      pts.push({ r: halfW * Math.cos(a), u: h / 2 + CUSTOM_ARCH * Math.sin(a) });
    }
 ...
 *
 * ГЛАВНОЕ ПРАВИЛО: ПЛОЩАДЬ КОНТУРА РАВНА ПЛОЩАДИ СЕЧЕНИЯ ВЕТВИ.
 *
 * Раньше контур строился только по габаритам (ширина, высота, свод), а поле
 * area игнорировалось — и это давало ровно ту ошибку, из-за которой две
 * выработки С ОДИНАКОВОЙ S на экране выглядели разными по толщине:
 *
 *   • S задана вручную (manualSection) или пришла из файла — а габариты
 *     остались дефолтными 4×3 со сводом 1 м от makeBranch. Выработка в 40 м²
 *     и выработка в 5 м² рисовались ОДНОЙ И ТОЙ ЖЕ трубой 4×3, потому что
 *     площадь никто не смотрел;
 *   • обратный случай: импорт (Excel) выводит габариты из периметра как
 *     квадрат со стороной P/4. У арки и у круга при одной и той же S периметр
 *     разный, значит и сторона квадрата разная — трубы получались разной
 *     толщины при равном сечении;
 *   • форма «custom» (импорт из АэроСети/ЕРП) габаритов не имеет вовсе и
 *     сваливалась в ветку «прямоугольник» с дефолтными числами: вся схема
 *     рисовалась выработками одного размера независимо от реального сечения.
 *
 * Поэтому здесь сначала берётся ФОРМА (пропорции стенок и свода — по ним арка
 * отличается от круга), а затем контур равномерно масштабируется так, чтобы
 * его площадь совпала с b.area — тем самым числом, по которому считается
 * вентиляция и по которому режим «Чертёж» подбирает толщину линии
 * (widthBySection: k = √(S/медиана) — тот же квадратный корень, что и здесь).
 *
 * Итог: одинаковая площадь — одинаковая толщина трубы, при любой форме и при
 * любом способе, которым сечение попало в проект. Форма при этом сохраняется:
 * круг остаётся кругом, арка — аркой, меняется только размер.
 */
export function sectionOutline(b: TopoBranch): SectionPoint[] {
  const pts = rawOutline(b);

  // Площадь, по которой считается вентиляция. Её и показываем.
  const target = b.area ?? 0;
  if (!(target > 0.01)) return pts;

  const raw = polygonArea(pts);
  if (!(raw > 1e-6)) return pts;

  // Масштаб по площади — корень: контур растягивается по обеим осям сразу,
  // поэтому площадь растёт как квадрат множителя.
  const k = Math.sqrt(target / raw);

  // Ограничитель на случай мусорных данных: сечение в тысячу раз больше или
  // меньше своих же габаритов — это ошибка ввода, а не выработка. Рисовать по
  // ней трубу во весь экран нельзя, схема станет нечитаемой.
  if (!(k > 0.02) || !(k < 50)) return pts;

  return pts.map(p => ({ r: p.r * k, u: p.u * k }));
}
