 ...
 *
 * 1,02 отвечает прогреву примерно на 6 °C. Такая поправка лежит внутри
 * погрешности исходных данных (сопротивления выработок задаются с куда
 * большим разбросом) и на расходы не влияет.
 *
 * ЗАЧЕМ ПОРОГ НУЖЕН ДЛЯ СКОРОСТИ. Каждое изменение сопротивления делает тело
 * запроса к решателю новым, и память расчётов (airflowCache) перестаёт
 * срабатывать. Из-за этого итерации, где физика уже не меняется, всё равно
 * уходили на сервер. С порогом едва тёплые ветви остаются нетронутыми, тело
 * запроса совпадает с предыдущим — и повторный расчёт не выполняется вовсе.
 */
export const THROTTLE_MIN_FACTOR = 1.02;
