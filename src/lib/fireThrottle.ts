...
//
// ─────────────────────────────────────────────────────────────────────────────
// ВНИМАНИЕ: ПОПРАВКА ВЫКЛЮЧЕНА ПО УМОЛЧАНИЮ — ДВОЙНОЙ УЧЁТ НАГРЕВА
//
// В нашей модели сети расширение горячего воздуха УЖЕ УЧТЕНО — в естественной
// тяге, которую решатель считает по фактическим температурам узлов:
//
//     h = g·Δz·(ρ_ветви − ρ_атм),   ρ = 353/(273 + T)
//
// Отношение плотностей ρ₀/ρ в точности равно отношению абсолютных температур
// T/T₀ — это одна и та же величина (при 20 → 237 °C обе равны 1,741). Поэтому
// множитель T/T₀, накинутый сверху на сопротивление, вводит нагрев ВТОРОЙ РАЗ.
//
// На восходящем проветривании это ломает расчёт: баланс сети смещается, струя
// ложно опрокидывается, расход растёт вместо падения (35,4 вместо 31,2 м³/с),
// а температура очага занижается (211 вместо 237 °C). Проверено на реальной
// схеме рудника.
//
// Код оставлен: он корректен для схем, где сопротивление задано при
// стандартной плотности, а тяга НЕ считается по фактическим температурам.
// Включать осознанно, понимая, какая модель тяги при этом используется.
//
// ─────────────────────────────────────────────────────────────────────────────
...
// правит руками в свойствах выработки.
//
// При выключении расчёт даёт в точности прежние числа — это проверяется
// тестом, чтобы нормативный режим нельзя было сломать случайно.
// ─────────────────────────────────────────────────────────────────────────────
...
/**
 * Порог значимости поправки: ниже него дроссель не применяется.
 *
 * 1,001 отвечает прогреву примерно на 0,3 °C — это заведомо ниже точности
 * задания температур, поэтому отбрасывается только численный шум.
 *
 * ПОЧЕМУ ПОРОГ НЕЛЬЗЯ ДЕЛАТЬ БОЛЬШИМ. Сначала здесь стояло 1,02 (прогрев
 * ~6 °C) — ради того, чтобы едва тёплые ветви не меняли тело запроса к
 * решателю и срабатывала память расчётов. Но на разветвлённой схеме таких
 * ветвей десятки, и отбрасывание поправки сразу на всех СМЕЩАЕТ БАЛАНС СЕТИ:
 * воздух перераспределяется иначе, а на восходящем проветривании это меняет
 * и момент опрокидывания струи. Экономия одного запроса не стоит искажения
 * расчёта, поэтому порог оставлен чисто «антишумовым».
 */
export const THROTTLE_MIN_FACTOR = 1.001;
...
/**
 * По умолчанию поправка ВЫКЛЮЧЕНА.
 *
 * ПОЧЕМУ. Проверка на реальной схеме показала двойной учёт нагрева.
 * Решатель сети уже учитывает расширение горячего воздуха — через плотность
 * в естественной тяге: h = g·Δz·(ρ_ветви − ρ_атм), где ρ = 353/(273+T).
 * Отношение плотностей ρ₀/ρ и отношение абсолютных температур T/T₀ — это
 * ОДНА И ТА ЖЕ величина (при 20 → 237 °C обе равны 1,741).
 *
 * Добавляя множитель T/T₀ к сопротивлению, поправка вводила тот же нагрев
 * второй раз. На восходящем проветривании это смещало баланс сети и приводило
 * к ложному опрокидыванию струи: расход рос вместо падения (35,4 против
 * 31,2 м³/с), а температура очага занижалась (211 вместо 237 °C).
 *
 * Поправка сохранена, но выключена: она остаётся верной для расчётных схем,
 * где сопротивление задано при стандартной плотности, а тяга не считается по
 * фактическим температурам узлов. Включать её осознанно, понимая, что модель
 * тяги в решателе при этом должна быть другой.
 */
export const DEFAULT_THROTTLE: ThrottleOptions = { enabled: false };
...
export function applyFireThrottle<T extends ThrottleBranchLite>(
  ...
  const out = branches.map(b => {
    ...
    // Ниже порога значимости ветвь НЕ трогаем: поправка тонет в погрешности
    // исходных данных, а изменённое сопротивление сделало бы тело запроса к
    // решателю новым и заставило пересчитывать то же самое заново.
    if (!(k >= THROTTLE_MIN_FACTOR)) return b;
