...
import { depressionPa } from "./resistanceUnits";
  ...
  for (const b of branches) {
    ...
    if (!descending && absAngle > FLAT_ANGLE_DEG && thermalDep > 0) {
      ...
      if (Q0 > 0) {
        ...
        // ЕДИНИЦЫ: R — Н·с²/м⁸, h_т — Па, поэтому R = h_т/Q₀² без переводных
        // множителей (см. resistanceUnits.ts). Раньше здесь стояло деление на
        // 9,81 под старое соглашение о кМюрг.
        R_calc = +(thermalDep / (Q0 * Q0)).toFixed(6);
        // (7.1) устойчиво, пока h_т < R·Q₀². Обе части — в паскалях.
        if (R_fact != null) {
          ...
          ascendingParallelRisk = thermalDep >= depressionPa(R_fact, Q0);
    ...
    // R·Q₀² выходит сразу в паскалях (R в Н·с²/м⁸), поэтому колонка h_кр и
    // показатель p_у сопоставимы с тепловой депрессией без пересчёта.
    const holdingDep_Pa = (!descending && R_fact != null && Q0_m3s != null)
      ? +depressionPa(R_fact, Q0_m3s).toFixed(1)
      : null;
