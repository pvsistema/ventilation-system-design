...
export const HIDDEN_LEGEND_IDS = new Set([
  ...
  // Дубли в группе «Аварии»: то же название, но без расчёта. Рабочие значки —
  // очаг пожара и место взрыва в «Аварийном режиме» (по ним идёт расчёт),
  // отделения ГСС в «Горноспасателях» (спрашивают численность), подземная
  // база там же. Из палитры убраны, но в старых проектах рисуются как раньше.
  ...
  // «Пожар» в «Тепле и газовыделении» — картинка без расчёта; расчётный
  // очаг — «Очаг пожара» в «Аварийном режиме».
  "heat_fire",
  // «Насосная станция стационарная» — картинка без расчёта; для водопровода
  // ставится «Насос» (напор, подача, направление).
  "pump_station",
  // «Взрыв» в «Аварийном режиме» — картинка без расчёта, повторяет по смыслу
  // расчётное «Место взрыва» той же группы.
  "acc_explosion",
  // «Двигатель внутреннего сгорания» в «Расчёте количества воздуха» —
...
/**
 * ID устьев и сечений стволов. Ставятся на ветвь и рисуются РОВНО ТОГО ЖЕ
 * размера, что и узел этой ветви — как в АэроСети. От вентиляторов не
 * зависят: у тех свой процент («Масштаб УО»), который к устью отношения
 * не имеет.
 */
/**
 * Коэффициент размера устья: значок делаем ровно с УЗЕЛ этой ветви.
 * Узел рисуется радиусом 0.55 от ширины ветви (canvasRenderer), то есть
 * диаметром 1.1·ширины. Круг внутри значка занимает 32 из 48 единиц
 * viewBox, поэтому саму картинку берём крупнее: 1.1 / (32/48) ≈ 1.65.
 */
export const SHAFT_MOUTH_NODE_K = 1.1 / (32 / 48);

/** Размер значка устья в пикселях по реальной ширине ветви на экране. */
export function shaftMouthSize(realBranchWpx: number, userScale = 1): number {
  return Math.max(6, realBranchWpx * SHAFT_MOUTH_NODE_K * userScale);
}
