...
export async function evaluateVariant(
  ...
  return {
    ...base,
    // Ошибка исходных данных (нет людей или выходов) — не повод считать,
    // что все спасены. Тогда сравнивать варианты по людям нельзя, и это
    // честно отражается нулём при непустом error в отчёте поиска.
