...
export interface FireAction {
  ...
  label: string;
  /**
   * Название объекта без глагола — «ВГП-1», «кв. 12».
   * Нужно, чтобы построить нормативную команду п.40, где название стоит
   * в середине фразы: «вентилятор ВГП-1 реверсировать».
   */
  objectName?: string;
  /** Вентилятор главного (вспомогательного) проветривания — см. underlineRed. */
  isMainFan?: boolean;
  /**
...
/**
 * Команда для оперативной части ПЛА — дословно по РД-15-11-2007, п.40.
 *
 * Документ задаёт формулировки буквально: «вентилятор ____ реверсировать»,
 * «вентилятор остановить». Наши подписи («Реверсировать вентилятор ВГП-1»)
 * удобны в окне подбора, но в план их переписывают в нормативном виде.
 *
 * Там же (п.40, п.31) закреплены ответственные: за работу вентустановок и
 * реверсивных устройств в аварийном режиме отвечают главный механик и
 * начальник ПВС, исполнители — машинисты вентустановок и диспетчер. Для
 * дверей и ляд ответственных назначают отдельно, поэтому текст другой.
 */
export interface RdCommand {
  /** Текст команды в нормативной формулировке. */
  text: string;
  /** Ответственные лица (графа 2 оперативной части). */
  responsible: string;
  /** Исполнители. */
  executor: string;
  /**
   * Команда касается ГЛАВНОЙ вентиляционной установки.
   * По п.40 такой текст в плане подчёркивают красной чертой — признак
   * нужен тому, кто формирует документ.
   */
  underlineRed: boolean;
}

/** Ответственные по п.40/п.31 — вентиляционные установки. */
const FAN_RESPONSIBLE = "Главный механик, начальник ПВС";
const FAN_EXECUTOR = "Машинист вентиляционной установки, диспетчер";
/** Двери, ляды и регуляторы — назначаются отдельным распоряжением. */
const DOOR_RESPONSIBLE = "Начальник ПВС";
const DOOR_EXECUTOR = "Горный мастер участка ВТБ";

/**
 * Перевести действие в нормативную команду плана ликвидации аварий.
 *
 * @param a       действие подбора
 * @param fanName название вентилятора или выработки — подставляется в пропуск
 *                формулировки «вентилятор ____ реверсировать»
 * @param isMain  вентилятор главного проветривания (ГВУ/ВВУ)
 */
export function toRdCommand(a: FireAction, fanName: string, isMain: boolean): RdCommand {
  switch (a.kind) {
    case "fan_reverse":
      return {
        text: `вентилятор ${fanName} реверсировать`,
        responsible: FAN_RESPONSIBLE, executor: FAN_EXECUTOR,
        underlineRed: isMain,
      };
    case "fan_stop":
      return {
        text: `вентилятор ${fanName} остановить`,
        responsible: FAN_RESPONSIBLE, executor: FAN_EXECUTOR,
        underlineRed: isMain,
      };
    case "fan_rpm":
      // Прямой формулировки для частотника РД не содержит (документ 2007 года),
      // поэтому строим её по образцу предыдущих: «вентилятор ___ перевести...».
      return {
        text: `вентилятор ${fanName} перевести на ${a.rpm ?? 0} об/мин`,
        responsible: FAN_RESPONSIBLE, executor: FAN_EXECUTOR,
        underlineRed: isMain,
      };
    case "door_close":
      return {
        text: `закрыть вентиляционную дверь ${fanName}`,
        responsible: DOOR_RESPONSIBLE, executor: DOOR_EXECUTOR, underlineRed: false,
      };
    case "door_open":
      return {
        text: `открыть вентиляционную дверь ${fanName}`,
        responsible: DOOR_RESPONSIBLE, executor: DOOR_EXECUTOR, underlineRed: false,
      };
    case "window_set":
      return {
        text: `установить вентиляционное окно ${fanName} площадью ${a.windowArea ?? 0} м²`,
        responsible: DOOR_RESPONSIBLE, executor: DOOR_EXECUTOR, underlineRed: false,
      };
  }
}
