...
export interface SymbolsInput {
  ...
  sizeK?: number;
  /**
   * Вентиляторы показываются объёмной моделью (см. mineFans.ts) — плоскую
   * карточку тогда ставить нельзя.
   *
   * Иначе у каждой машины появился бы двойник: карточка со значком, висящая
   * поперёк собственного кожуха. Решение принимает вызывающая сторона: она
   * знает, включён ли объёмный показ вентиляторов.
   */
  skipFans?: boolean;
  /**
   * Замерные станции показываются объёмной обоймой (см. mineMeasureStations.ts)
   * — плоскую карточку тогда ставить нельзя.
   *
   * Иначе у каждой станции появился бы двойник: красная заслонка поперёк
   * выработки внутри собственной обоймы. А заслонка у замерной станции — прямая
...
export function buildMineSymbols(input: SymbolsInput): MineSymbols | null {
  ...
  for (const sym of symbols) {
    ...
    // Вентилятор в объёме — не карточка, а машина: кожух, лопасти, вращение
    // (mineFans.ts). Плоский значок ему в этом режиме не нужен.
    if (input.skipFans && sym.typeId === "fan") continue;
