...
// линии ВДОЛЬ выработки, разведённые поперёк неё, — и ничего больше. Никаких
// перекладин на концах: любая черта ПОПЕРЁК ветви возвращает знаку вид
// заслонки, а границы участка замера видны по концам самих линий. Между
// линиями натянута лёгкая красная плёнка ВДОЛЬ выработки — она показывает сам
// участок и служит крупной мишенью для курсора.
//
// ДЛИНА ОДНА НА ВСЮ СХЕМУ. Раньше она считалась от сечения выработки: на
// стволе станция вытягивалась, на сбойке съёживалась, и знаки одного смысла
// выглядели разными. Теперь длина одна (STATION_LEN_M) и подрезается только
// долей длины самой ветви — чтобы на короткой выработке знак не доставал до
// её узлов.
//
...
/**
 * Брусков в одном знаке: ровно две линии вдоль выработки. Перекладин на концах
 * больше нет — любая линия ПОПЕРЁК ветви возвращала знаку вид заслонки, а
 * границы участка замера и без них видны по концам самих линий.
 */
const BARS_PER_STATION = 2;

/**
 * ОДНА длина участка замера для всей схемы, в метрах. Раньше длину считали от
 * сечения выработки: на стволе 8×8 станция вытягивалась на десятки метров, на
 * сбойке оставалась коротышкой — знаки одного смысла выглядели разными. Теперь
 * длина у всех одна, как у знака на чертеже.
 */
const STATION_LEN_M = 6;

/**
 * Насколько знак может занять свою выработку. Участок замера — часть ветви, а
 * не вся она: на короткой сбойке длину подрезаем до этой доли, иначе станция
 * растягивается от узла до узла и перестаёт читаться как отметка на выработке.
 */
const MAX_BRANCH_SHARE = 0.3;

/** Цвет замерной станции — тот же красный, что у знака на чертеже (#dc2626). */
const COLOR = new THREE.Color(0xdc2626);
/** Под курсором линии светлеют: видно, к какой станции относится подсказка. */
...
export function buildMineMeasureStations(
  ...
  /** Заготовка одного знака: положение и размер каждого её бруска. */
  interface Bar {
  ...
  for (const sym of symbols) {
    ...
    // Длина участка замера — ОДНА для всех станций схемы (STATION_LEN_M),
    // масштаб знака и ручка S/M/L её только подкручивают. От сечения она больше
    // не зависит: на стволе знак вытягивался, на сбойке съёживался, и одинаковые
    // по смыслу станции читались разными. Подрезаем долей длины самой ветви —
    // на короткой выработке знак не должен доставать до её узлов.
    const want = STATION_LEN_M * kx * sc * sizeK;
    const hl = Math.max(0.3, Math.min(want, len * MAX_BRANCH_SHARE)) / 2;
    ...
    // Перекладин на концах больше нет. Они были короткие, только между
    // линиями, и всё равно давали ПОПЕРЕЧНУЮ черту — на схеме это опять
    // читалось как перекрытая выработка. Границы участка замера видны по
    // концам самих линий, соединять их нечем.
    ...
    // берём ось X сцены — иначе тройка вырождается и знак схлопывается.
  ...
  // ── Линии: все бруски всех станций одной пакетной отрисовкой ──────────
  ...
  const barMat = new THREE.MeshStandardMaterial({
    ...
    // Свечение: линии обязаны читаться и в тёмной глубине схемы, и на
  ...
  // Поверх тела выработки: линии идут внутри неё, и в сплошном режиме
  ...
  const pick = (ray: THREE.Raycaster): MeasureStationHit | null => {
    // Ловим и по линиям, и по плёнке между ними: плёнка — самая крупная часть
