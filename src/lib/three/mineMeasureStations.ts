...
//
// ЧТО ЗДЕСЬ. Станция собирается объёмной: участок выработки длиной L берётся в
// красную обойму — две рамки по сечению на концах участка и четыре прогона
// ВДОЛЬ оси между ними. Линии обоймы идут вдоль ветви, как на чертеже, а
// торцы участка обозначены полупрозрачными красными окнами — это те самые
// сечения, по которым считают расход.
//
// СКОРОСТЬ. Вся обойма — прямоугольные бруски: одна пакетная отрисовка
// (InstancedMesh) на все станции схемы, плюс одна на торцевые окна. Сколько бы
...
/** Брусков в одной обойме: 4 прогона вдоль оси + две рамки по 4 бруска. */
const BARS_PER_STATION = 12;
...
export function buildMineMeasureStations(
  ...
  interface Station {
    ...
    zAxis: THREE.Vector3;
    bars: Bar[];
    /** Полуразмеры сечения обоймы и половина длины участка. */
  ...
  for (const sym of symbols) {
    ...
    // Обойма чуть шире сечения: линии должны идти ПО стенкам выработки, а не
    // внутри неё — внутри они тонули бы в теле трубы и в сплошном режиме
    // пропадали вовсе.
    const hw = dims.w * kx * 0.58;
    const hh = dims.h * kx * 0.58;
    // Длина участка замера. На чертеже это отрезок вдоль выработки, и в
    // объёме он обязан быть заметно длиннее своего сечения — иначе обойма
    // выглядит кубиком и снова читается как перегородка. Ограничиваем третью
    // длины самой выработки: участок замера не может занять её целиком.
    const want = Math.max(dims.w, dims.h) * kx * 2.6 * sc * sizeK;
    const hl = Math.max(0.3, Math.min(want, len * 0.34)) / 2;

    // Толщина прутка — от меньшего размера сечения: на сбойке 2×2 брусок в
    // те же сантиметры, что на стволе, был бы не виден.
    const th = Math.max(0.06, Math.min(hw, hh) * 0.16);

    const bars: Bar[] = [];
    // ── Четыре прогона ВДОЛЬ ветви ──────────────────────────────────────
    // Это и есть те самые две красные линии чертежа, только в объёме их
    // четыре — по углам сечения, чтобы участок читался с любого ракурса.
    for (const sx of [-1, 1]) {
      for (const sy of [-1, 1]) {
        bars.push({ ox: sx * hw, oy: sy * hh, oz: 0, sx: th, sy: th, sz: hl * 2 });
      }
    }
    // ── Две рамки по сечению — торцы участка замера ──────────────────────
    // Ровно те сечения, между которыми ведётся замер. Они не перекрывают
    // выработку: это рамка по контуру, а не заслонка.
    for (const sz of [-1, 1]) {
      const z = sz * hl;
      bars.push({ ox: 0, oy: hh, oz: z, sx: hw * 2 + th, sy: th, sz: th });
      bars.push({ ox: 0, oy: -hh, oz: z, sx: hw * 2 + th, sy: th, sz: th });
      bars.push({ ox: hw, oy: 0, oz: z, sx: th, sy: hh * 2 - th, sz: th });
      bars.push({ ox: -hw, oy: 0, oz: z, sx: th, sy: hh * 2 - th, sz: th });
    }
    ...
    stations.push({
      ...
      pos, xAxis, yAxis, zAxis, bars, hw, hh, hl,
    });
  }
  ...
  // ── Торцевые окна: сечения, по которым считают расход ──────────────────
  const panelGeom = new THREE.PlaneGeometry(1, 1);
  const panelMat = new THREE.MeshBasicMaterial({
    color: COLOR, transparent: true, opacity: 0.14,
    side: THREE.DoubleSide, depthWrite: false, toneMapped: false,
  });
  const panelMesh = new THREE.InstancedMesh(panelGeom, panelMat, stations.length * 2);
  ...
  stations.forEach((st, si) => {
    ...
    [-1, 1].forEach((sz, pi) => {
      center.copy(st.pos).addScaledVector(st.zAxis, sz * st.hl);
      m.makeBasis(st.xAxis, st.yAxis, st.zAxis);
      m.scale(new THREE.Vector3(st.hw * 2, st.hh * 2, 1));
      m.setPosition(center);
      panelMesh.setMatrixAt(si * 2 + pi, m);
    });
  });
  ...
  const pick = (ray: THREE.Raycaster): MeasureStationHit | null => {
    ...
    for (const h of hits) {
      ...
      const si = h.object === barMesh
        ? Math.floor(i / BARS_PER_STATION)
        : Math.floor(i / 2);
