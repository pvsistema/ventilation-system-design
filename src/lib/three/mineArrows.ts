...
function planArrows(input: ArrowsInput): { plan: Placement[]; total: number } {
  ...
  for (const b of branches) {
    ...
    //
    // Число стрелок — сколько таких шагов укладывается в свободную часть.
    // Ноль недопустим: выработка с расходом обязана показывать направление,
    // даже если короче шага, — там ставим одну стрелку.
    const step = pitch;
    const count = Math.max(1, Math.min(MAX_PER_BRANCH, Math.round(free / step)));
...
export function buildFlowArrows(input: ArrowsInput): FlowArrows | null {
  ...
  for (const pl of plan) {
    ...
    // ── Пробег (длина цикла) ─────────────────────────────────────────
    //
    // ЗДЕСЬ И БЫЛ «ТРИГГЕР» В КОРОТКИХ ВЫРАБОТКАХ. Раньше пробег равнялся
    // свободной длине самой выработки: на сбойке в пару метров стрелка
    // проскакивала цикл за доли секунды и мельтешила, а на штреке шла плавно.
    // Разный цикл = разная частота, и единого движения воздуха не получалось.
    //
    // Теперь пробег равен ШАГУ между стрелками, общему на всю схему. Стрелка
    // доходит ровно до места, где её сменяет следующая, и со стороны это
    // выглядит как непрерывная цепочка, идущая насквозь. Цикл при этом
    // одинаков у всех выработок — частота мелькания больше не зависит от
    // длины.
    //
    // На выработке короче шага цикл укорачивается до её свободной длины:
    // иначе стрелка ушла бы за узел, в зону сопряжения с соседними телами.
    const free = pl.len - pl.margin * 2;
    const spanWorld = Math.max(pl.aLen * 0.25, Math.min(pl.step, free - pl.aLen * 0.15));
    ...
    for (let i = 0; i < pl.count; i++) {
      ...
      // Две составляющие.
      //
      // Первая — место стрелки в цепочке: каждая следующая отстаёт на шаг,
      // поэтому цепочка выглядит сплошной, а не пачкой, вышедшей разом.
      //
      // Вторая — привязка к ОБЩЕМУ НУЛЮ схемы. Без неё каждая выработка
      // начинала бы отсчёт от своего начала, и на стыке двух выработок
      // стрелки шли бы вразнобой: в одной только что прошла, в другой ещё на
      // подходе. Сдвигаем фазу на расстояние от общего начала координат до
      // начала выработки — тогда цепочки соседних выработок продолжают друг
      // друга, и поток читается как сквозной.
      // Знак у второй составляющей минусовой: сдвиг должен КОМПЕНСИРОВАТЬ
      // положение начала выработки, чтобы стрелки всех выработок встали на
      // одну и ту же сетку в мировых координатах. Плюс сдвинул бы их в ту же
      // сторону, что и саму выработку, и сетки бы не совпали.
      const chain = spanLocal * (i / pl.count);
      const startAlongDir = pl.from.dot(pl.dir) + pl.margin;
      ...
      // хватает с запасом.
      const raw = chain - startAlongDir / pl.aLen;
      aPhase[idx] = ((raw % spanLocal) + spanLocal) % spanLocal;
      idx++;
    }
  }
