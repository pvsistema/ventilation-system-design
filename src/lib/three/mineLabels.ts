...
export function drawMineLabels(
  ...
  for (const L of labels) {
    ...
    let offX = 0, offY = 0;
    if (L.manualOffset) {
      // Оттащено руками на чертеже — переносим как есть, экранными пикселями.
      offX = L.offX; offY = L.offY;
    } else {
      ...
      let nx = -dy, ny = dx;
      if (ny > 0) { nx = -nx; ny = -ny; }
      // Отступ — полблока текста плюс зазор: подпись не должна касаться трубы.
      const gap = bh / 2 + 6;
      offX = nx * gap; offY = ny * gap;
    }
    const sx = mx + offX;
    const sy = my + offY;
