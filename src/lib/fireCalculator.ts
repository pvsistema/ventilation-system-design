...
export function computeHotNodeTemps(
  ...
  //
  // Поэтому вводим нормативное затухание по ПРОЙДЕННОМУ ПУТИ: перегрев узла
  // умножается на e^(−x/l) и за пределами зоны горения быстро сходит на нет —
  // ровно как задумано методикой. Масштабом спада служит сама длина зоны
  // горения l (4.8), а не l·A из показателя (4.12): см. разбор в decayScaleOf.
  const decayScaleOf = (fb: { flow: number; originalFlow?: number; area?: number }) => {
    ...
    // ── МАСШТАБ СПАДА — ДЛИНА ЗОНЫ ГОРЕНИЯ, А НЕ l·A ────────────────────────
    //
    // Здесь стояло scale = l·A, взятое из показателя (4.12) x̄/A = x/(l·A).
    // На практике это давало 230–250 м: перегрев доживал до исходящего ствола
    // почти целым (на 100 м — 36 % от +167 °C), и решатель набирал на сотнях
    // метров вертикали +300…+600 Па тяги дымовой трубы. В нисходящей выработке
    // это перебивало тормоз очага, и расход РОС на единицы м³/с вместо того,
    // чтобы чуть упасть (в Аэросети на той же схеме −0,4 м³/с).
    //
    // Методика продлевать горячий столб за зону горения не разрешает: (4.6)
    // прямо ограничивает плечо депрессии величиной Δz = l·sinβ. За пределами
    // зоны дым перемешан со свежим воздухом, тепло ушло в массив, и тяговым
    // столбом он больше не работает. Берём scale = l: на границе зоны остаётся
    // e⁻¹ ≈ 37 % перегрева, дальше добивает второй сомножитель в decayAt.
    //
    // Тепло самой зоны горения при этом не теряется — оно учтено напрямую
    // депрессией h_t, которую ветвь очага отдаёт решателю источником.
    return { l, scale: l };                                // зона горения и масштаб спада, м
  };
