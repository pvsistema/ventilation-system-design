...
export function calcThermalDepressionNormative(
  ...
  // ИСПРАВЛЕНО. Раньше стояло lCol = min(l, x) — столб обрезался ДЛИНОЙ ЗОНЫ
  // ГОРЕНИЯ l. Но l (4.8) зависит только от t, Q и S и на длинных выработках
  // упирается в свой потолок (при t=150 мин, Q=50, S=12 → l≈86 м). В итоге на
  // любой ветви длиннее ~90 м столб был один и тот же, h_т не зависела от x, и
  // очаг у входа, в середине и у выхода давал ОДИНАКОВУЮ депрессию — нисходящая
  // ветвь опрокидывалась независимо от того, куда поставлен очаг. Это и есть
  // та самая жалоба: «норматив 4.5 переворачивает нисходящее проветривание
  // независимо от места очага».
  //
  // Физика: за пределами зоны горения продукты не исчезают — они продолжают
  // течь к устью, ОСТЫВАЯ о стенки, и всё это время создают тягу. Поэтому столб
  // тянется на всю длину x, а его вклад взвешивается долей вытеснения
  // θ(s) = 1 − T₀/T(s) в каждой точке:
  //
  //     l_эфф = (1/θ₀)·∫₀ˣ θ(s) ds ,  θ₀ = 1 − T₀/Tм
  //     T(s) = T₀ + (Tм−T₀)·e^(−(s/l)/A)              при s ≤ l   (4.12)
  //     T(s) = T₀ + (T(l)−T₀)·e^(−α·P·(s−l)/(ṁ·cp))   при s > l   (остывание)
  //
  // Такой столб МОНОТОННО растёт с x: очаг у входа даёт максимум, у устья — 0.
  const sinB = Math.sin((beta * Math.PI) / 180);
  const theta0 = Tm > Tamb ? 1 - Tamb / Tm : 0;
  let lCol = 0;
  if (theta0 > 0 && x > 0) {
    const per = 4 * Math.sqrt(S);                 // периметр по площади сечения
    const massFlow = Math.max(0.5, 1.25 * Q);     // кг/с
    const kWall = (WALL_HEAT_ALPHA * per) / (massFlow * CP_AIR * 1000); // 1/м
    const Tend = Tamb + (Tm - Tamb) * Math.exp(-1 / A);  // T на границе зоны l
    const N = 200;
    const ds = x / N;
    let acc = 0;
    for (let i = 0; i <= N; i++) {
      const s = i * ds;
      const Ts = s <= l
        ? Tamb + (Tm - Tamb) * Math.exp(-(s / l) / A)
        : Tamb + (Tend - Tamb) * Math.exp(-kWall * (s - l));
      acc += (i === 0 || i === N ? 0.5 : 1) * (1 - Tamb / Ts);
    }
    lCol = Math.min(x, (acc * ds) / theta0);
  }
  ...
  // перепадом высот, но ПРОПОРЦИОНАЛЬНО пройденному газом пути «очаг → устье»:
  // участок длиной x из выработки L даёт перепад не больше drop·(x/L).
  //
  // ИСПРАВЛЕНО. Раньше доля считалась по lCol/L. Из-за потолка lCol (см. выше)
  // эта доля тоже переставала зависеть от положения очага, и ограничение
  // «замораживало» Δz на одном значении для любого x. Долю задаёт именно x —
  // расстояние от очага до устья, то есть реальный перепад отметок, который
  // проходит горячая струя.
