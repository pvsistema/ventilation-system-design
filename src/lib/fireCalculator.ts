...
// ─────────────────────────────────────────────────────────────────────────────
// ЕДИНИЦЫ ДЕПРЕССИИ. Депрессия ветви b.dP приходит из расчёта сети в ПАСКАЛЯХ:
// сопротивление R хранится в Н·с²/м⁸, поэтому произведение R·Q² — это сразу
// паскали, без переводных множителей (см. lib/resistanceUnits.ts).
...
export function calcThermalDepressionNormative(
  ...
  // СВЕРЕНО С ПО «Вентиляция» по двум контрольным точкам диалога «Установка
  // очага пожара» (ветвь 465: L=76.2 м, S=6.5 м², Q=33.29 м³/с, β=−89.8°,
  // T очага 1000 °C, T₀=20 °C):
  //
  //   t=54 мин,  x=34.3 м → зона горения 34.5 м, Ht = −31.7 даПа (−317 Па)
  //   t=119 мин, x=76.2 м → зона горения 76.0 м, Ht = −70.4 даПа (−704 Па)
  //
  // Формула lCol = min(l, x) даёт −320 и −708 Па — совпадение в пределах
  // округления. Промежуточный вариант с интегралом остывания за границей зоны
  // горения давал −132 Па на 500-метровой ветви вместо нормативных значений и
  // от эталона уходил, поэтому здесь остаётся ровно нормативная трактовка:
  // тягу создаёт ЗОНА ГОРЕНИЯ, а не весь путь газа до устья.
  //
  // Физический смысл: за пределами зоны горения продукты уже остыли настолько,
  // что вклад в тягу пренебрежим, — норматив 4.8 именно это и закладывает,
  // ограничивая длину горящего участка временем развития пожара.
  ...
  // перепадом высот, но ПРОПОРЦИОНАЛЬНО длине самого столба: участок lCol из
  // выработки L даёт перепад не больше drop·(lCol/L). Так ограничение не
  // «обгоняет» формулу (4.6) и не гасит зависимость от положения очага —
  // сравнивается ровно тот отрезок, который и создаёт тягу.
...
export function calcCriticalDepression(inp: CriticalDepInput): CriticalDepResult {
  ...
  // ЕДИНИЦЫ. Сопротивление r_п — Н·с²/м⁸, поэтому произведение r·Q² в формулах
  // (5.3)–(5.5) даёт паскали СРАЗУ, и h_кр сопоставима с тепловой депрессией
  // пожара без пересчёта. Множитель 9,81, стоявший здесь под старое соглашение
  // о кМюрг, убран вместе с самим соглашением (см. lib/resistanceUnits.ts).
...
export function calcFireSeatDepression(
  ...
  //
  // ВАЖНО: x считается ТОЛЬКО по фактическому положению очага (fireT) и длине
  // ветви. Раньше глобальная ручная настройка «Очаг→устье x» перебивала этот
  // расчёт (userMouth > 0 ? userMouth : auto), и получалось противоречие: очаг
  // двигают по ветви — x не меняется, а введённые вручную 1–2 м делали любую
  // выработку устойчивой независимо от того, где стоит очаг. Положение очага
  // задаётся ползунком в ветви (как в ПО «Вентиляция»), и x всегда следует
  // за ним.
...
export function calcFireMode(
  ...
  for (const fb of fireBranches) {
    ...
    // Берётся ВСЕГДА из геометрии: доля ветви от очага (fireT) до выхода в
    // направлении потока. Очаг двигается ползунком по ветви — x следует за ним.
    //
    // Ручная глобальная настройка «Очаг→устье x» здесь БОЛЬШЕ НЕ ПЕРЕБИВАЕТ
    // авторасчёт: одно число на всю схему не может описывать положение очага в
    // конкретной ветви. Из-за прежнего приоритета получалось, что очаг возят по
    // выработке, а депрессия не меняется, зато введённые вручную 1–2 м делали
    // любую ветвь устойчивой.
    //
    // x = 0 — ЗАКОННОЕ значение: очаг у самого устья, столб горячих газов
    // вырождается, тяги нет.
    const fireTpos = fb.fireT ?? 0.5;
    const outFrac = dirFlow >= 0 ? (1 - fireTpos) : fireTpos;
    const mouthDist = (fb.length ?? 0) * outFrac;
