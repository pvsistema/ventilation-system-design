...
export function computeHotNodeTemps(
  ...
  //
  // УБРАН ОПОРНЫЙ РАСХОД ПЛЮМА (был fireRefFlow = штатный расход очага, и по
  // нему считались и остывание, и вес струи в смешении).
  //
  // Почему это было грубой ошибкой. Температура продуктов горения считается как
  // T = T₀ + N/(ρ·cp·Q_расч), то есть привязана к некоторому расчётному расходу.
  // А в узел смешения та же струя входила с массой по ШТАТНОМУ расходу, даже
  // когда фактический расход схлопнулся. Энергия, вброшенная в сеть, равна
  // m·cp·(T−T₀) = N·(Q_штат/Q_расч): при Q_факт 15 м³/с против штатных 75
  // расчётный расход брался 37,5, масса — 75, и очаг 8,5 МВт превращался в узле
  // в 17 МВт. Ровно вдвое завышенный подогрев столба → завышенная тяга →
  // ложное опрокидывание смежных выработок.
  //
  // Теперь масса горячей струи — ФАКТИЧЕСКАЯ (ρ·Q_факт данной ветви), как и у
  // любой другой ветви в балансе. Это и есть закон сохранения массы: сколько
  // воздуха реально идёт через очаг, столько и выходит из него дальше по сети.
  //
  // Плотность транспорта берём ρ₀ = 1,25 кг/м³ ОСОЗНАННО, а не ρ(T) горячей
  // струи. Решатель сети несжимаемый: расход Q вдоль пути дыма не «распухает»
  // при нагреве. Масса, которую переносит струя, — это масса ХОЛОДНОГО воздуха,
  // вошедшего в очаг, она вдоль пути сохраняется. Подставь мы сюда ρ(400 °C),
  // масса по ходу нагрева уменьшилась бы втрое из ничего — это нарушило бы
  // сохранение массы вместо того, чтобы его восстановить.
  const RHO_TRANSPORT = 1.25;   // кг/м³ — та же опорная плотность, что в calcFireTemp
  ...
  for (const fb of fireBranches) {
    ...
    // Масса горячей струи — по ФАКТИЧЕСКОМУ расходу ветви очага (было: не ниже
    // штатного, см. разбор у RHO_TRANSPORT). Нижняя отсечка 0,4 м³/с — только
    // защита от деления на ноль на практически стоящей струе, а не «подпорка»
    // расхода до штатного.
    const m = RHO_TRANSPORT * Math.max(0.4, Math.abs(fb.flow ?? dirFlow));
  ...
  for (let it = 0; it < 60; it++) {
    ...
    branchesByOutNode.forEach((incoming, nodeId) => {
      ...
      for (const b of incoming) {
        ...
        const m = RHO_TRANSPORT * Math.max(0.4, flow);
  ...
  // ── ГРАНИЦА ТЯГОВОГО СТОЛБА: ТОЛЬКО ЗОНА ГОРЕНИЯ ──────────────────────────
  //
  // ИСПРАВЛЕНИЕ ГЛАВНОЙ ПРИЧИНЫ РОСТА РАСХОДА ПРИ НИСХОДЯЩЕМ ПРОВЕТРИВАНИИ.
  //
  // Что было. Тяга набиралась на ВСЁМ пути дыма: ветвь очага тормозила струю
  // на своей геометрической Δz (31 м · sin10° = 5,4 м → всего −11 Па), а
  // остывший дым, поднимаясь по стволу в сотни метров, работал дымовой трубой
  // и давал +30…+120 Па В ПОМОЩЬ потоку. Итог арифметики — не торможение, а
  // разгон: 35 → 40 м³/с, ровно то, что видно на схеме. Плавного затухания
  // (decayAt) для этого мало: на стволе 300 м даже 8 К остаточного перегрева
  // дают +36 Па, а это больше всей тормозящей тяги очага.
  //
  // Что по методике. Приложение 4 считает тепловую депрессию на разности
  // отметок ЗОНЫ ГОРЕНИЯ Δz = l·sinβ (4.6) и НИГДЕ не продлевает её дальше.
  // Приложение 2 (форм. 2.2) ставит её одним слагаемым: h = h_т + R_у·Q_в²,
  // то есть при нисходящем проветривании h_т может только СНИЖАТЬ расход.
  // Никакого выигрыша от трубы на исходящей стороне методика не даёт.
  //
  // Поэтому вес столба греем только внутри зоны горения, а саму h_т прикладываем
  // сосредоточенно на ветви очага (см. fireModeRun.ts). Карта hot остаётся полной —
  // она нужна задымлению, газам и опасности, а не тяге.
  const seatIds = new Set(fireBranches.map(fb => fb.id));
  const outsideBurnZone = (nid: string) =>
    decayParams != null && (distFromFire.get(nid) ?? Infinity) > decayParams.l;
  ...
  for (const b of allBranches) {
    ...
    // Ветвь очага: её вклад в тягу учтён сосредоточенной h_т по (4.5), поэтому
    // тяговый столб по температурам ей не начисляем — иначе двойной счёт.
    // Ветви за пределами зоны горения: дым уже перемешан со свежим воздухом,
    // тепло ушло в массив — столб не тяговый.
    if (seatIds.has(b.id) || outsideBurnZone(inNode)) {
      const r2z = (v: number) => Math.round(v * 100) / 100;
      branchTemps[b.id] = { tFrom: r2z(baseOf(b.fromId)), tTo: r2z(baseOf(b.toId)) };
      continue;
    }
