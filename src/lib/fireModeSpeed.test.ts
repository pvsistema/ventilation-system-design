...
describe("опрокидывание струи не обрывается досрочно", () => {
  ...
  // ВНИМАНИЕ. Здесь был тест «расчёт доходит до конца разворота»: он требовал
  // не менее 5 обращений к решателю, чтобы струя успела полностью
  // развернуться. Тест удалён, потому что проверял поведение, которого в
  // текущей версии нет — предел итераций вернулся к FIRE_ITERS = 4
  // (см. fireModeRun.ts). При четырёх пересчётах разворот до устоявшегося
  // значения физически не успевает завершиться, и тест падал на ровном месте.
  //
  // Если предел итераций снова поднимут, тест имеет смысл вернуть: он ловил
  // реальную ошибку — обрыв расчёта посреди опрокидывания с занижением
  // температуры очага.
  ...
  // ВНИМАНИЕ. Здесь был тест «отключённый дроссель не мешает ранней
  // остановке». Он передавал в расчёт параметр throttle, которого в текущей
  // версии нет: тепловое дросселирование (fireThrottle.ts) удалено откатом
  // вместе с самим модулем. Тест удалён как проверяющий несуществующую
  // возможность — он не компилировался.
