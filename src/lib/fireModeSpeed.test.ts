...
describe("скорость расчёта пожарного режима", () => {
  ...
  it("колебания без сходимости: срабатывает выход по стагнации", async () => {
    // Расход скачет между двумя значениями и никогда не сходится.
    // Без выхода по стагнации цикл дошёл бы до предела в 12 итераций.
    let calls = 0;
    const solveIteration = async (brs: TopoBranch[]) => {
      calls += 1;
      const m = new Map<string, number>();
      for (const b of brs) {
        m.set(b.id, b.id === "b1" ? (calls % 2 === 0 ? 5 : 25) : 20);
      }
      return m;
    };
