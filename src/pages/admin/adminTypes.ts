...
export interface OfflineKey {
  ...
  /**
   * Ключ сам закрепляется за компьютерами, которые первыми выйдут на связь
   * (в пределах числа мест). Нужен там, где код рабочего места спросить не у
   * кого — удалённый рудник без связи.
   */
  autobind?: boolean;
  /** Когда сработала автопривязка (ключ закрепился за компьютером) */
  bound_at?: string | null;
  /** Имя компьютера, за которым ключ закрепился автоматически */
  bound_host?: string | null;
  /** Номер ключа, выпущенного взамен этого (перевыпуск с привязкой к ПК) */
  replaced_by_id?: number | null;
  /** Когда ключ отозван */
  revoked_at?: string | null;
  /**
   * Код рабочего места, которое уже отмечалось по ключу. Подставляется в поле
   * привязки при перевыпуске — чтобы код ПК не пришлось узнавать у человека.
   */
  seat_fp?: string | null;
}
