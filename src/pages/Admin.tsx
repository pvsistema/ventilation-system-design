...
export default function Admin() {
  ...
  /**
   * Перевыпуск аварийного ключа с привязкой к компьютеру.
   *
   * ЗАЧЕМ. Ключи, выданные без привязки, работают на любой машине: файл можно
   * скопировать на сколько угодно компьютеров, и без интернета это ничем не
   * ограничено. Перевыпуск создаёт ключ, привязанный к коду рабочего места, а
   * старый сразу отзывает — с этого момента он не проходит даже при подстановке
   * в запрос напрямую, минуя программу.
   *
   * ВАЖНО для человека за панелью: старый ключ перестаёт работать немедленно,
   * поэтому новый нужно передать людям сразу. Об этом предупреждаем явно.
   */
  const reissueOffline = async (k: OfflineKey) => {
    // Если по ключу уже выходил на связь компьютер, привязку возьмёт сервер
    // сам — из реестра рабочих мест. Спрашивать код у человека не нужно.
    const knowsPc = k.used_seats > 0;
    const hint = knowsPc
      ? "Компьютер известен: по ключу уже выходили на связь. Привязка будет "
        + "сделана к нему автоматически — оставьте поле пустым.\n\n"
        + "Либо введите код другого рабочего места (строка «ID …» в окне «Лицензия»)."
      : "Введите код рабочего места — он показан в программе в окне «Лицензия» "
        + "(строка «ID …», 8 символов).";

    const fp = prompt(
      `Перевыпуск аварийного ключа «${k.org}» с привязкой к компьютеру.\n\n`
      + hint + "\n\n"
      + "ВНИМАНИЕ: старый ключ будет отозван немедленно. Новый ключ нужно "
      + "передать людям сразу, иначе работа у них остановится.",
      "",
    );
    if (fp === null) return;
    const clean = fp.trim().replace(/[^0-9A-Fa-f]/g, "").toUpperCase();
    // Пусто — привязку подберёт сервер по реестру. Но если компьютер ни разу
    // не выходил на связь, брать её неоткуда: просим ввести код.
    if (!clean && !knowsPc) {
      alert("Нужен код рабочего места: по этому ключу ещё ни один компьютер "
        + "не выходил на связь, и взять привязку автоматически неоткуда.");
      return;
    }
    if (clean && clean.length < 6) {
      alert("Код рабочего места слишком короткий. Нужны 8 символов из окна «Лицензия».");
      return;
    }
    try {
      const r = await adminApi(password, {
        action: "reissue_offline_key", offline_key_id: k.id, bound_fp: clean,
      });
      loadOfflineKeys(password);
      const res = r as { key?: string; id?: number; bound_fp?: string };
      // Показываем новый ключ сразу: его нужно скопировать и передать.
      setOkShowKeyId(res.id ?? null);
      const boundShown = (res.bound_fp ?? clean).slice(0, 8);
      alert(
        `Готово. Ключ #${k.id} отозван, выпущен новый #${res.id}.\n\n`
        + `Привязан к компьютеру: ${boundShown}…\n`
        + "Работать будет только на нём.\n\n"
        + "Новый ключ раскрыт в списке — скопируйте его и передайте на рабочее место.",
      );
    } catch (e: unknown) {
