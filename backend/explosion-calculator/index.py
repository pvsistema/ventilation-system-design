...
def junction_transmission(area_in, area_out_total):
    """Коэффициент прохождения ДАВЛЕНИЯ через сопряжение выработок.

    Раньше давление делилось по сечениям (split = S_i/ΣS), что неверно:
    ΔP — величина интенсивная, она не делится. Делится поток энергии, а
    фронт проходит в каждую ветвь почти целиком, теряя часть на отражении
    от разрыва площади. Прежняя модель гасила волну за 5-6 узлов, и
    окраска обрывалась в середине схемы.

    Акустическая модель (скачок импеданса, для канала Z ~ 1/S):
        tau = 2*S_вх / (S_вх + ΣS_исх)
    Для типового тройника tau = 2/3 вместо прежних 0.5.
    """
    if area_in <= 0 or area_out_total <= 0:
        return 1.0
    return min(1.0, (2.0 * area_in) / (area_in + area_out_total))
