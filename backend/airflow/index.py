...
def build_graph(nodes_in, branches_in, surface_temp=20.0, geo_gradient=0.0,
    ...
    for b in branches_in:
        ...
        # ── ВЕТВЬ ОЧАГА С ГОТОВОЙ ДЕПРЕССИЕЙ — СТОЛБ НЕ СЧИТАЕМ ──────────────
        #
        # Если фронтенд прислал тепловую депрессию зоны горения (h_fire по
        # формулам 4.5–4.6), то вес горячего столба ЭТОЙ ветви уже учтён в ней.
        # Считать его здесь второй раз по температурам концов — двойной учёт.
        #
        # Разница не косметическая: методика берёт Δz по длине ЗОНЫ ГОРЕНИЯ
        # (l·sinβ), а здешняя формула — по геометрии ветви (z_from − z_to). Это
        # разные плечи, и складывать их нельзя. Оставляем ветви очага обычную
        # (геотермическую) тягу по температурам узлов, а весь тепловой эффект
        # приносит h_fire.
        _seat_with_h = bool(b.get("hasFire")) and abs(
            float(b.get("fireThermalDepression", 0) or 0)) > 1e-9
        if t_from_br is not None and t_to_br is not None:
            if _seat_with_h:
                # Оба конца — по ВХОДЯЩЕЙ (ещё не нагретой) струе. Иначе горячий
                # конец пришёл бы сюда через температуру узла и добавил столб
                # поверх h_fire. Ветвь очага для решателя остаётся холодной:
                # весь её тепловой эффект уже принесён источником.
                t_cold = min(float(t_from_br), float(t_to_br))
                ft = tt = t_cold
            else:
                ft = float(t_from_br)
                tt = float(t_to_br)
        ...
        h_nat = natural_draft_h(
            ...
            # экспоненте остывания (см. разбор в natural_draft_h). Если
            # депрессия зоны горения пришла готовой (_seat_with_h), ветвь уже
            # сделана изотермической выше и ступенька ни на что не влияет.
            is_fire_seat=bool(b.get("hasFire")),
        )

        # ── ТЕПЛОВАЯ ДЕПРЕССИЯ ПОЖАРА (Па) ───────────────────────────────────
        # Депрессия ЗОНЫ ГОРЕНИЯ по формулам (4.5)–(4.6): Δz = l·sinβ, где l —
        # длина зоны горения (4.8), а β — угол выработки ОТНОСИТЕЛЬНО ПОТОКА.
        # Считается фронтендом и приходит готовой; здесь просто добавляется к
        # естественной тяге ветви как сосредоточенный источник.
        #
        # Знак уже учитывает и уклон, и направление проветривания:
        #   + восходящая выработка → пожар усиливает тягу (как доп. вентилятор)
        #   − нисходящая           → пожар тормозит / опрокидывает поток
        #
        # Плечо здесь НЕ равно перепаду отметок ветви: зона горения обычно
        # длиннее самой выработки. Поэтому на ветви очага тепловой столб по
        # температурам не считается — иначе те же метры учлись бы дважды.
