...
def natural_draft_h(from_z, to_z, from_temp, to_temp, atm_temp=None,
                    ...
                    use_humidity=False, is_fire_seat=False):
    """
    ...
    #
    # ПРОФИЛЬ ЗАВИСИТ ОТ ТОГО, ГДЕ ВЕТВЬ СТОИТ ОТНОСИТЕЛЬНО ОЧАГА. Это ключевой
    # момент, на котором предыдущая правка ошиблась и сделала хуже.
    #
    #   • ВЕТВЬ ОЧАГА (is_fire_seat): очаг сидит примерно в СЕРЕДИНЕ выработки.
    #     До него идёт холодный воздух, после — горячие продукты. Профиль
    #     СТУПЕНЧАТЫЙ, а не плавный: полдлины при T_вх, полдлины при T_вых.
    #     Вес столба = (ρ(T_вх) + ρ(T_вых))/2.
    #     Экспонента здесь категорически неприменима: она предполагает, что газ
    #     УЖЕ горячий на входе и только остывает. Подстановка экспоненты для
    #     входа 15 °C и выхода 182 °C дала ρ = 1,145 вместо 1,001 — то есть
    #     столб объявлялся почти холодным, тормозящая тяга нисходящего съезда
    #     падала с −16 до −5 Па, и расход рос ещё сильнее (35 → 42 м³/с).
    #
    #   • ВЕТВИ ЗА ОЧАГОМ (обычные): газ входит горячим и остывает о стенки по
    #     закону t(x) = t_фон + Δt_вх·e^(−kx) — тот же, что в модели дыма
    #     (computeHotNodeTemps). Вес столба = ∫ρ по этому профилю; в замкнутом
    #     виде (подстановка u = Δt):
    #       ρ_ср = 353/(A·ln(Δt₁/Δt₂)) · ln[Δt₁·(A+Δt₂)/(Δt₂·(A+Δt₁))], A = 273+t_фон.
    #
    # Зачем это вообще нужно. Вес столба — ИНТЕГРАЛ плотности по высоте, а
    # ρ = 353/(273+t) выпукла по температуре, поэтому ρ(t_ср) ≠ среднему ρ и на
    # сотнях градусов сильно занижает вес. Ствол 140 м, низ 127,7 °C, верх
    # 18,4 °C: ρ(t_ср) даёт тягу дымовой трубы 282 Па, честный ∫ρ — 153 Па.
    # Завышенный вдвое подсос со стороны ствола и переворачивал знак итога при
    # нисходящем проветривании.
    #
    # Во всех НЕпожарных случаях (геотермия, нагорные рудники, изотермия)
    # возвращается ровно прежняя ρ(t_ср) — штатные расчёты не меняются.
    # Коррекцию вносим МНОЖИТЕЛЕМ ρ_столба/ρ(t_ср) по сухой формуле, чтобы не
    # конфликтовать с учётом влажности по (9.2).
    if rho_mean > 0:
        th_a = float(from_temp) - t_atm
        th_b = float(to_temp) - t_atm
        # Порог 5 °C по перепаду вдоль ветви: ниже него разница между ρ(t_ср) и
        # ∫ρ лежит за пределами значащих цифр, и трогать штатные схемы незачем.
        if max(th_a, th_b) > 5.0 and abs(th_a - th_b) > 5.0:
            A = 273.0 + t_atm
            if is_fire_seat:
                # Ступенька: половина выработки холодная, половина горячая.
                rho_col = 0.5 * (353.0 / (A + max(0.0, th_a))
                                 + 353.0 / (A + max(0.0, th_b)))
            else:
                hi, lo = (th_a, th_b) if th_a >= th_b else (th_b, th_a)
                # Холодный конец мог слиться с фоном — экспонента там не
                # определена; берём малую, но конечную долю перегрева.
...
def build_graph(nodes_in, branches_in, surface_temp=20.0, geo_gradient=0.0,
    ...
    for b in branches_in:
        ...
        h_nat = natural_draft_h(
            ...
            use_humidity=use_humidity,
            # Ветвь с очагом: нагрев происходит СКАЧКОМ в середине выработки,
            # поэтому вес её столба считается по ступенчатому профилю, а не по
            # экспоненте остывания (см. разбор в natural_draft_h).
            is_fire_seat=bool(b.get("hasFire")),
        )
