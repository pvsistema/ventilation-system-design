...
def get_R(b):
    """Сопротивление ветви, Н·с²/м⁸.

    ЕДИНИЦЫ. Весь решатель считает ΔP = R·Q² прямо в ПАСКАЛЯХ, без множителя g:
    R приходит с фронта в Н·с²/м⁸, там же выражены напор вентилятора,
    естественная тяга и тепловая депрессия пожара (см. src/lib/resistanceUnits.ts).
    Поле fanCrossingR клиент тоже присылает уже переведённым — раньше оно
    приходило в кМюрг и складывалось с СИ-сопротивлением ветви, из-за чего
    перемычка ГВУ учитывалась почти в десять раз слабее.
    """
    ...
    if b.get("hasFan") and b.get("fanInstall", "Внутри перемычки") == "Внутри перемычки":
        ...
        r += crossing_r
    # Минимум 1e-4 — физический предел крупного ствола (30 м² × 1000 м).
