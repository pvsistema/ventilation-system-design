...
def natural_draft_h(from_z, to_z, from_temp, to_temp, atm_temp=None,
    ...
    # ── ВЕС ГОРЯЧЕГО СТОЛБА: СРЕДНЯЯ ПЛОТНОСТЬ, А НЕ ПЛОТНОСТЬ ПРИ СРЕДНЕЙ T ──
    #
    # Вес столба — это ИНТЕГРАЛ ПЛОТНОСТИ по высоте: p = g·∫ρ(z)dz. Плотность
    # ρ = 353/(273+t) — функция ВЫПУКЛАЯ по температуре, поэтому ρ(t_ср) НЕ равно
    # среднему значению ρ по столбу и всегда ЗАНИЖАЕТ его, когда температура по
    # длине меняется сильно. На холодных перепадах (единицы градусов) разница
    # ничтожна, а на пожаре она становится решающей.
    #
    # ЧИСЛА. Ствол 140 м, низ 127,7 °C (горячий дым из сопряжения), верх 18,4 °C
    # (дым уже остыл), фон 15 °C:
    #   ρ(t_ср=73 °C)          = 1,020 → тяга «дымовой трубы»  282 Па
    #   ∫ρ по экспоненте       = 1,114 → тяга                  153 Па
    # То есть прежняя формула давала ВДВОЕ завышенный подсос. Именно он и
    # переворачивал знак итога: при НИСХОДЯЩЕМ проветривании тормозящий горячий
    # столб в наклонном съезде (−24 Па) тонул в фиктивных +282 Па со стороны
    # ствола, и расход не падал, как требует методика, а РОС (35 → 39 м³/с).
    #
    # Профиль берём тот же, что в модели остывания дыма (computeHotNodeTemps):
    #   t(x) = t_фон + Δt_вх·e^(−kx)  — перегрев спадает экспоненциально.
    # Тогда ∫ρ имеет замкнутый вид (подстановка u = Δt):
    #   ρ_ср = 353/(A·ln(Δt₁/Δt₂)) · ln[ Δt₁·(A+Δt₂) / (Δt₂·(A+Δt₁)) ],  A = 273+t_фон.
    #
    # Применяем ТОЛЬКО к перегретому столбу (оба конца выше опорной температуры
    # больше чем на 0,5 °C) — то есть к пути дыма пожара. Во всех остальных
    # случаях (геотермия, нагорные рудники, изотермия) возвращается ровно
    # прежняя ρ(t_ср), поэтому штатные расчёты не меняются ни на цифру.
    # Коррекцию вносим МНОЖИТЕЛЕМ ρ_столба/ρ(t_ср), посчитанным по сухой формуле.
    # Так поправка на профиль температуры не конфликтует с учётом влажности по
    # (9.2): влажностная часть остаётся ровно той, что вернул air_density.
    if rho_mean > 0:
        th_a = float(from_temp) - t_atm
        th_b = float(to_temp) - t_atm
        # Порог 5 °C по ПЕРЕПАДУ вдоль ветви — страховка обратной совместимости.
        # При равномерном столбе формула ниже тождественно равна ρ(t_ср), так что
        # геотермия (6 °C на стволе 600 м) не сдвинется даже в четвёртом знаке;
        # условие лишь гарантирует, что штатные схемы считаются бит в бит как
        # раньше, а новая ветка кода включается только на пожарном столбе.
        if max(th_a, th_b) > 5.0 and abs(th_a - th_b) > 5.0:
            hi, lo = (th_a, th_b) if th_a >= th_b else (th_b, th_a)
            A = 273.0 + t_atm
            # Холодный конец столба. Если он уже слился с фоном (или даже чуть
            # ниже), экспоненциальный профиль формально не определён — берём
            # малую, но конечную долю перегрева. Это КЛЮЧЕВОЙ случай для ветви
            # очага при нисходящем проветривании: греется только выходной узел,
            # входной остаётся фоновым, и без такой отсечки вес столба снова
            # считался бы по ρ(t_ср), то есть завышенно лёгким.
            lo = max(lo, hi * 1e-3, 0.05)
            if hi - lo < 1e-6:
                rho_col = 353.0 / (A + hi)
            else:
                rho_col = (353.0 / (A * math.log(hi / lo))) * math.log(
                    (hi * (A + lo)) / (lo * (A + hi)))
            rho_dry_mean = 353.0 / (273.0 + t_mean)
            if rho_dry_mean > 0:
                rho_mean *= rho_col / rho_dry_mean
...
def solve_mkr(nodes_in, branches_in, options, normal_flows=None, surface_temp=20.0,
    ...
    def sync_tree_q(Q_arr):
        ...
        for idx in range(len(bfs_order) - 1, 0, -1):
            ...
            b  = bal[v]
            # Вентилятор, попавший в ДЕРЕВО (fallback, когда обходного пути без
            # него нет). Раньше его расход не пересчитывался из баланса: он
            # оставался со значением, накопленным итерациями, а невязка узла
            # просто передавалась родителю. В узле подключения оставалась
            # разница — тот самый дисбаланс при последовательных ВМП, каждый
            # из которых уходил в дерево.
            #
            # Физически древесная ветвь ОДНОЗНАЧНО определяется 1-м законом
            # Кирхгофа: сколько принесли соседние ветви, столько она и несёт —
            # независимо от того, стоит на ней вентилятор или нет. Напор
            # вентилятора при этом учитывается в невязках контуров, куда эта
            # ветвь входит. Поэтому пересчитываем её так же, как обычную.
            #
            # Исключение — ГВУ/ВВУ на границе с атмосферой: их расход задаёт
            # характеристика, а не баланс (иначе главный вентилятор «поплывёт»).
            if (
                e.get("hasFan") and not e.get("fanStopped")
                and e.get("fanType", "ГВУ") != "ВМП"
            ):
                bal[p_node] += b
                continue
